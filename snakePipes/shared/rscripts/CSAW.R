...
method<-snakemake@params[["csaw_counts_method"]]
...
message(paste("windowSize :", windowSize, "\n"))
...
sampleInfo$name = gsub("allele_specific/","",sampleInfo$name)
## allelic_info: if running comparison between WT and KO for each allele
allelic_info = allelic_info & length(unique(sampleInfo$condition)) == 1
...
if(snakemake@params[['peakCaller']] == "MACS2") {
    ...
    allpeaks <- lapply(fnames, function(x) {
        narrow <- paste0("../MACS2/",gsub(".genome[0-9]","",x),bam_pfx,".BAM_peaks.narrowPeak") #bam_pfx
        if(snakemake@params[["pipeline"]] %in% "ATAC-seq"){
            narrow <- paste0("../MACS2/",gsub(".genome[0-9]","",x),".filtered.short.BAM_peaks.narrowPeak")
        }
        broad <- paste0("../MACS2/",gsub(".genome[0-9]","",x),bam_pfx,".BAM_peaks.broadPeak") #bam_pfx
    ...
    if ( (length(grep("genome1", sampleInfo$name)) != 0 | length(grep("genome2", sampleInfo$name)) != 0) ){
        allpeaks.allelic <- lapply(fnames, function(x) {
            narrow <- paste0("../MACS2/allele_specific/",x,bam_pfx,".BAM_peaks.narrowPeak") #bam_pfx
            if(snakemake@params[["pipeline"]] %in% "ATAC-seq"){
                narrow <- paste0("../MACS2/allele_specific/",x,".filtered.short.BAM_peaks.narrowPeak")
            }
            broad <- paste0("../MACS2/allele_specific/",x,bam_pfx,".BAM_peaks.broadPeak") #bam_pfx
            
            # first look for narrowpeak then braod peak
            if(file.exists(narrow)) {
                message(narrow)
                bed <- try(read.delim(narrow, header = FALSE), silent = TRUE)
            } else if (file.exists(broad)) {
                message(broad)
                bed <- try(read.delim(broad, header = FALSE), silent = TRUE)
            } else {
                stop("MACS2 output doesn't exist. Neither ", narrow, " , nor ", broad)
            }
            
            if(attr(bed,"class") != "try-error"){bed.gr <- GRanges(seqnames = bed$V1, ranges = IRanges(start = bed$V2, end = bed$V3), name = bed$V4)} else {bed.gr = GRanges()}
            return(bed.gr)
        })
        allpeaks = c(allpeaks, allpeaks.allelic)
    }
    
} else {
    
    allpeaks = lapply(snakemake@input[['peaks']], function(x) {
        bed = read.delim(paste0("../", gsub("_genome[0-9]","",x)), header=FALSE)
        message(bed)
        bed.gr = GRanges(seqnames = bed$V1, ranges = IRanges(start = bed$V2, end = bed$V3), name = bed$V4)
        return(bed.gr)
    })
    ...
    if ( (length(grep("genome1", sampleInfo$name)) != 0 | length(grep("genome2", sampleInfo$name)) != 0) ){
        allpeaks.allelic = lapply(snakemake@input[['peaks']], function(x) {
            bed = try(read.delim(paste0("../", x), header=FALSE), silent = TRUE)
            message(bed)
            if(attr(bed,"class") != "try-error"){bed.gr = GRanges(seqnames = bed$V1, ranges = IRanges(start = bed$V2, end = bed$V3), name = bed$V4)} else {bed.gr = GRanges()}
            return(bed.gr)
        })
...
allpeaks$name = paste0("peak_", 1:length(allpeaks))
names(allpeaks) = allpeaks$name

###MACS peak counts and CSAW  window counts
chip_object <- readfiles_chip(sampleSheet = sampleInfo,
                              fragmentLength = fraglength,
                              window_size = windowSize,
                              alleleSpecific = allelic_info,
                              pe.param = pe_param, peak.files = allpeaks)
...
chip_results <- getDBregions_chip(chip_object, plotfile = "DiffBinding_modelfit")

## write output
message("Writing output")
writeOutput_chip(chip_results, outfile_prefix = "DiffBinding", fdrcutoff = fdr, lfccutoff=lfc, method = method)
